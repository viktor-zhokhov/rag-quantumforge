...
- **Как заменяли термины:** скрипт `scripts/replace_terms.py` читает словарь `knowledge_base/terms_map.json` (131 пара «исходное → вымышленное»), заменяет по границам слов от длинных ключей к коротким и пишет анонимизированные документы в `knowledge_base/`.
...
- **Результат:** 2960 замен, ни одного узнаваемого термина Star Wars в финальной базе (проверено grep-ом).
